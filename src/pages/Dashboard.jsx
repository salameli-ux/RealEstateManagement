...
const defaultRevenueData = [
  { name: 'Jan', revenue: 12800, occupancy: 88 },
  { name: 'Feb', revenue: 13700, occupancy: 91 },
  { name: 'Mar', revenue: 14550, occupancy: 93 },
  { name: 'Apr', revenue: 15300, occupancy: 95 },
  { name: 'May', revenue: 16400, occupancy: 96 },
]

const formatCurrency = (amount) => `$${amount.toLocaleString()}`

const buildRevenueChartData = (payments) => {
  const monthIndex = {}
  payments.forEach((payment) => {
    ...
    if (!monthIndex[key]) {
      monthIndex[key] = { name: label, revenue: 0, occupancy: 88 }
    }
  ...
  const sorted = Object.entries(monthIndex)
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([_, value], index) => ({
      ...value,
      occupancy: Math.min(98, value.occupancy + index),
    }))

  return sorted.length ? sorted : defaultRevenueData
}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    fetchPayments()
      .then((data) => {
        setSummary(buildPaymentSummary(data))
        setChartData(buildRevenueChartData(data))
      })
      .catch((error) => {
        console.error('Dashboard payment fetch failed', error)
      })
  }, [])
        ...
        <div className="card chart-card">
          <div className="card-header">
            <div>
              ...
              <p>Portfolio utilization by month.</p>
            </div>
            <span className="pill">Top market: NYC</span>
          </div>
